...
export const load: PageServerLoad = async () => {
	// 1. Profiles (always exists for any user we created).
	...
	if (profErr) {
		throw error(500, profErr.message);
	}

	// 2. Auth users (for last_sign_in_at — purely informational).
...
export const actions: Actions = {
	create: async ({ request }) => {
		...
		const categories = role === 'judge' ? parseCategories(form) : (['A', 'B', 'C'] as Category[]);
