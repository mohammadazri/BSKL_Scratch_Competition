...
5. Add a CNAME `judging → <UUID>.cfargotunnel.com` (proxied) in the Cloudflare
   ...
   `cloudflared tunnel route dns p3-judging p3.sentrizk.me`.
6. Start everything:
   ```bash
...
8. Smoke-check from outside: open `https://p3.sentrizk.me`.
...
sudo cloudflared --config /etc/cloudflared/config.yml ingress rule https://p3.sentrizk.me
```
