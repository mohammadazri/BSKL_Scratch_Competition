...
const FAKE_SCHOOLS = [
	{ name: `${SEED_TAG} BSKL`, shortCode: 'BSKL' },
	{ name: `${SEED_TAG} ISKL`, shortCode: 'ISKL' },
	{ name: `${SEED_TAG} MKIS`, shortCode: 'MKIS' },
	{ name: `${SEED_TAG} Garden`, shortCode: 'GARDEN' },
	{ name: `${SEED_TAG} Alice Smith`, shortCode: 'ALICE' }
];

const FAKE_JUDGES = [
	{ email: `judge1${SEED_DOMAIN}`, fullName: 'Judge One', pinLabel: 'Sticky #1' },
	{ email: `judge2${SEED_DOMAIN}`, fullName: 'Judge Two', pinLabel: 'Sticky #2' },
	{ email: `judge3${SEED_DOMAIN}`, fullName: 'Judge Three', pinLabel: 'Sticky #3' },
	{ email: `judge4${SEED_DOMAIN}`, fullName: 'Judge Four', pinLabel: 'Sticky #4' }
];

const FAKE_VIEWER = {
	email: `viewer1${SEED_DOMAIN}`,
...
const PARTICIPANTS_PER_CATEGORY = 15;
...
function makeParticipantName(catIdx: number, n: number): string {
	return `${SEED_TAG} Participant ${CATEGORIES[catIdx]}${String(n).padStart(2, '0')}`;
}
...
async function deleteSeededAuthUsers(admin: SupabaseClient): Promise<void> {
	...
	for (let page = 1; page <= 20; page++) {
		...
		for (const u of data.users) {
			if (u.email && u.email.toLowerCase().endsWith(SEED_DOMAIN)) {
				targets.push(u.id);
			}
		}
...
async function deleteSeededTables(admin: SupabaseClient): Promise<void> {
	// Participants cascade to scoresheets, scores, DQ flags via FK ON DELETE.
	// Schools restrict-delete if participants still reference them; the
	// participant delete above clears the way.
	...
	const { error: sErr } = await admin.from('schools').delete().like('name', `${SEED_TAG}%`);
	if (sErr) throw sErr;
	console.log(`[seed-fake-event] removed stale seeded participants + schools`);
}
...
async function main(): Promise<void> {
	...
	// Phase 1 — wipe stale seeded rows so the run is idempotent.
